...
-- Some other Department are added later manually by edit  **20.01.2017
...
WHERE Printer_Computers_Informations.Information_Model_ID is NULL; --509  

SELECT * FROM AssetDataAbrasives
SELECT * FROM OriginalAssetDataAbrasivesMatch
--SELECT * FROM OriginalAssetDataAbrasivesPrinter
--SELECT * FROM OriginalSWListAbrasives
SELECT * FROM Asset

--SELECT * FROM OriginalPrinterAbrasives
...
INSERT INTO dbo.Printer_Computers_Informations (SerialNumber,Location,Department)
SELECT SerialNumber , Location,Department
FROM OriginalAssetDataAbrasivesPrinter --28 
...
HAVING COUNT(*) >1
--******
SELECT DISTINCT (MACAddress) FROM Printers_Computers
--18.01.2017
-- after adding Prod_ID FK key we need to import data into it , to finde the ProdID FROM OriginalAssetDataAbrasivesMatch  
--we use following code

SELECT * FROM dbo.Printers_Computers
ORDER BY Model_ID_FK , SerialNumber ASC 

SELECT ModelCategory, Model, Manufacturer, SerialNumber,Product_ID_IFK 
...
-- in order to import data of VendorID_FK and  ProdID_FK we have to export the excel file then edit tit again import it
